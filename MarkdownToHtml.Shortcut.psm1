...
Function Set-MarkdownToHtmlShortcut {
  ...
  csc.exe /nologo /target:library $(If ($HideConsole) { '/define:HIDE_CONSOLE' }) /define:DLL_LIBRARY /out:$(($LauncherDll = [Path]::ChangeExtension($PSCommandPath, '.Launcher.dll'))) $(($CsScript = Set-ConvertMd2HtmlExtension '.cs'))
  csc.exe /nologo /target:winexe /reference:$LauncherDll /out:$(($ConvertExe = Set-ConvertMd2HtmlExtension '.exe')) $CsScript
  $Env:Path = $EnvPath
